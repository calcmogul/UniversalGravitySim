...
float BoxToSFML_y(float y, float boxHeight) { return boxHeight - y * 30.f; }

sf::Vector2f BoxToSFML(float x, float y, float boxHeight) {
    return sf::Vector2f(x * 30.f, boxHeight - y * 30.f);
}

float SFMLToBox_x(float x) { return x / 30.f; }

float SFMLToBox_y(float y, float boxHeight) { return (boxHeight - y) / 30.f; }
...
Box2DBase::Box2DBase(sf::Shape* sfShape, const sf::Vector2f& position,
                     b2BodyType bodyType) {
    b2BodyDef bodyDef;
...
void Box2DBase::syncObject(const sf::Window& referTo) {
    ...
    m_objectPath.at(m_objectPath.size() - 1)
        ->setPosition(drawShape->getPosition());
    m_objectPath.at(m_objectPath.size() - 1)
