...
int main() {
    sf::RenderWindow mainWin(
        sf::VideoMode::getDesktopMode(), "Universal Gravitation Simulator",
    ...
    mainWin.setIcon(appIcon.getSize().x, appIcon.getSize().y,
    ...
    backgroundSprite.setTextureRect(sf::IntRect(
        -86, -86, mainWin.getSize().x + 2 * 86, mainWin.getSize().y + 2 * 86));
    ...
    mainWin.setView(sf::View(
        sf::FloatRect(0.f, 0.f, mainWin.getSize().x, mainWin.getSize().y)));
    ...
    while (mainWin.isOpen()) {
        while (mainWin.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {
                ...
                backgroundSprite.setTextureRect(
                    sf::IntRect(-86.f, -86.f, mainWin.getSize().x + 2.f * 86.f,
                                mainWin.getSize().y + 2.f * 86.f));

                HUDBackground.setScale(mainWin.getSize().x / 50.f,
                                       HUDBackground.getScale().y);
        ...
        if (!isPaused) {
            ...
            Box2DBase::world.Step(timeStep, velocityIterations,
                                  positionIterations);
        ...
        if (mainWin.getView().getCenter().x - backgroundSprite.getPosition().x <
            ...
            backgroundSprite.setPosition(
                backgroundSprite.getPosition().x - 86.f,
        ...
        mainWin.setView(sf::View(sf::FloatRect(
            myShip.shape.getPosition().x - mainWin.getSize().x / 2,
