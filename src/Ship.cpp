...
Ship::Ship(const sf::Vector2f& position, float fullHealth)
    ...
    shipVertices[1].Set(-15.f / 30.f,
                        (m_shipTexture.getSize().y - 36.f) / 30.f);
    shipVertices[2].Set(-15.f / 30.f,
                        (m_shipTexture.getSize().y - 51.f) / 30.f);
...
float Ship::getHealth() { return m_health; }
